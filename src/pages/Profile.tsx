...
import { LogOut, BookOpen, Heart, Users, TrendingUp, Pencil, Camera } from "lucide-react";
...
const Profile = () => {
  ...
  const [profile, setProfile] = useState<{ display_name: string; avatar_url: string | null; bio: string } | null>(null);
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { error: uploadError } = await supabase.storage.from("community-photos").upload(path, file, { upsert: true });
    if (uploadError) { toast({ title: t("general.error"), description: uploadError.message, variant: "destructive" }); return; }
  ...
  const saveProfile = async () => {
    if (!user) return;
    setSaving(true);
    const { error } = await supabase.from("profiles").update({ display_name: editName, bio: editBio } as any).eq("user_id", user.id);
    if (error) {
            ...
            <CardContent className="p-4 -mt-10">
              <div className="flex items-end gap-3">
                <button onClick={() => fileInputRef.current?.click()} className="relative group">
                  <div className="h-16 w-16 rounded-2xl bg-card shadow-lg flex items-center justify-center text-2xl font-display font-bold text-primary border-4 border-card overflow-hidden">
                    {profile?.avatar_url ? (
