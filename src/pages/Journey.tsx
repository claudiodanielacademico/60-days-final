...
const Journey = () => {
  ...
  // Calculate how many days are unlocked based on signup date
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      // Calculate unlocked days from user created_at
  ...
  const markComplete = async (dayNumber: number) => {
    ...
    if (error) {
      ...
      toast({ title: "Day completed! ✝️", description: "Keep walking with the Lord." });
    }
  };
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mx-auto max-w-lg p-4 space-y-6">
          <div className="text-center">
            <span className="inline-flex items-center rounded-full bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
              Day {selectedDay.day_number} of 60
            </span>
          ...
          <Card className="border-0 bg-primary/5 shadow-sm">
            <CardContent className="p-5">
              <h3 className="mb-2 text-sm font-semibold text-primary">Today's Task</h3>
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="sticky top-0 z-10 border-b border-border bg-card/90 backdrop-blur-md px-4 py-4">
        <h1 className="font-display text-xl font-bold">My Journey</h1>
        <div className="mt-3 space-y-1">
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            <span>Day {completedDays} of 60</span>
      ...
      <div className="mx-auto max-w-lg space-y-2 p-4">
        {loading ? (
          <div className="py-20 text-center text-muted-foreground">Loading your journey...</div>
        ) : content.length === 0 ? (
          <div className="py-20 text-center text-muted-foreground">
            <BookOpen className="mx-auto mb-3 h-10 w-10 text-muted-foreground/50" />
            <p>Devotional content is being prepared.</p>
            <p className="mt-1 text-xs">Check back soon!</p>
          </div>
        ) : (
          content.map((day, i) => {
              ...
              >
                <button
                  ...
                  className={cn(
                    ...
                    unlocked ? "bg-card shadow-sm hover:shadow-md" : "bg-muted/50 opacity-60",
                ...
                >
                  <div
                    className={cn(
                      ...
                      completed ? "bg-green-100 text-green-700" : isCurrent ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"
                    )}
                  >
