-- ==========================================================
-- SCRIPT DE CONSERTO ABSOLUTO E DEFINITIVO (NUCLEAR)
-- Este script reconstrói a base com permissões ULTRA-PERMISSIVAS para o dono.
-- ==========================================================

BEGIN;

-- 1. LIMPEZA TOTAL (Sem chance de erro de dependência)
...
-- 2. CRIAÇÃO DAS TABELAS COM ESTRUTURA BLINDADA
...
-- 3. SEGURANÇA (RLS) - "OWNER CAN DO EVERYTHING"
-- Aqui matamos o erro de RLS.

ALTER TABLE public.profiles ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Public Profiles" ON public.profiles FOR SELECT USING (true);
CREATE POLICY "Owner All Prof" ON public.profiles FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

ALTER TABLE public.daily_content_library ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Public Content" ON public.daily_content_library FOR SELECT USING (true);

ALTER TABLE public.journey_progress ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Owner Progress" ON public.journey_progress FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- 4. LOGICA DE PERFIL AUTOMÁTICA
...
-- 5. POPULAÇÃO DOS 60 PASSOS (CONTEÚDO REAL)
...
(52, 'Verdade Livre', 'Conhecer a verdade é ser livre.', 'João 8:32', 'Abra hoje a Biblia e sinta a paz.', 'Tua palavra é a verdade.', 1),
...
(55, 'Paciência', 'A paciência produz caráter.', 'Romanos 5:3-4', 'Mantenha a calma em um imprevisto.', 'Dá-me paciência.', 1),
...
(60, 'Vida Plena', 'Tudo se fez novo.', '2 Coríntios 5:17', 'Comemore! Você concluiu os 60 passos de hoje.', 'Eis que tudo se fez novo.', 1);

-- 6. BIBLIOTECA ANUAL (Povoamento Forçado)
...
SELECT v_id, day_number, title, reflection, scripture, task, prayer, duration_min, 'Standard'
FROM public.daily_content, generate_series(1, 365) AS v_id;

-- 7. SINCRONIZA USUÁRIOS
INSERT INTO public.profiles (user_id, display_name, username, user_code)
SELECT 
    id, 
    COALESCE(email, 'Usuário'), 
